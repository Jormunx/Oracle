...
--下面是创建用户new_user_129
```
CREATE USER new_user_129 IDENTIFIED BY 123
DEFAULT TABLESPACE "USERS"
TEMPORARY TABLESPACE "TEMP";

-- QUOTAS
ALTER USER new_user_129 QUOTA UNLIMITED ON USERS;
ALTER USER new_user_129 QUOTA UNLIMITED ON USERS02;
ALTER USER new_user_129 ACCOUNT UNLOCK;

-- ROLES
GRANT "CONNECT" TO new_user_129 WITH ADMIN OPTION;
GRANT "RESOURCE" TO new_user_129 WITH ADMIN OPTION;
ALTER USER new_user_129 DEFAULT ROLE "CONNECT","RESOURCE";

-- SYSTEM PRIVILEGES
GRANT CREATE VIEW TO new_user_129 WITH ADMIN OPTION;
```
...
begin
  ...
  loop
    ...
    ------------------------插入订单y一个订单包括3个产品4个订单详表--------------------------------
    v:=dbms_random.value(10000,4000);
    v_name:='computer'|| (i mod 4 + 1);
    insert /*+append*/ into ORDER_DETAILS(ID,ORDER_ID,PRODUCT_NAME,PRODUCT_NUM,PRODUCT_PRICE)
      values (SEQ_ORDER_DETAILS_ID.NEXTVAL,v_order_id,v_name,2,v);
    v:=dbms_random.value(1000,50);
    v_name:='paper'|| (i mod 4 + 1);
    insert /*+append*/ into ORDER_DETAILS(ID,ORDER_ID,PRODUCT_NAME,PRODUCT_NUM,PRODUCT_PRICE)
      values (SEQ_ORDER_DETAILS_ID.NEXTVAL,v_order_id,v_name,3,v);
    v:=dbms_random.value(9000,2000);
    v_name:='phone'|| (i mod 4 + 1);
    insert /*+append*/ into ORDER_DETAILS(ID,ORDER_ID,PRODUCT_NAME,PRODUCT_NUM,PRODUCT_PRICE)
...
```
#### 插入数据及修改结果：
##### ![图片加载失败](./images/0.2.png)

###  查询数据：
    1.查询某个员工的信息
    
   ![图片加载失败](./images/1.1.png)
   
    2.递归查询某个员工及其所有下属，子下属员工。
    
   ![图片加载失败](./images/2.1.png)
   
    3.查询订单表，并且包括订单的订单应收货款: Trade_Receivable= sum(订单详单表.ProductNum*订单详单表.ProductPrice)- Discount。
    
   ![图片加载失败](./images/3.0.png)
      ![图片加载失败](./images/3.1.png)
   ![图片加载失败](./images/3.2.png)
   
   
    4.查询订单详表，要求显示订单的客户名称和客户电话，产品类型用汉字描述。
    
   ![图片加载失败](./images/4.1.png)
    
    5.查询出所有空订单，即没有订单详单的订单。
    
   ![图片加载失败](./images/5.1.png)
   
    6.查询部门表，同时显示部门的负责人姓名。
    
   ![图片加载失败](./images/6.1.png)
   
    7.查询部门表，统计每个部门的销售总金额。
    
   ![图片加载失败](./images/7.1.png)
